...
export function NotificationList({ items, compact = false }: { items: Notification[]; compact?: boolean }) {
  return (
          ...
          <form action={openNotificationAction}>
            ...
            >
              ...
              <span className="flex min-w-0 flex-1 flex-col gap-0.5">
                ...
                {item.body && <span className="text-muted-foreground line-clamp-2 text-xs font-normal">{item.body}</span>}
                <time dateTime={item.createdAt.toISOString()} className="text-muted-foreground text-xs font-normal">
                  {ago(item.createdAt)}
                </time>
              </span>
            </Button>
